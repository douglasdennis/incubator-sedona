...
class geoparquetIOTests extends TestBaseScala with BeforeAndAfterAll {
  ...
  val geometryCol = "geometry"
  val g0Col = "g0"
  val g1Col = "g1"
  val nameCol = "name"
  val geoparquetFormat = "geoparquet"
  val geoMetadataName = "geo"

  def getGeomTypes(geo: JValue, column: String): Seq[String] = {
    implicit val formats : org.json4s.Formats = org.json4s.DefaultFormats
    (geo \ "columns" \ column \ "geometry_types").extract[Seq[String]]
  }

  describe("GeoParquet IO tests"){
    it("GEOPARQUET Test example1 i.e. naturalearth_lowers dataset's Read and Write"){
      val df = sparkSession.read.format(geoparquetFormat).load(geoparquetdatalocation1)
      val rows = df.collect()(0)
      assert(rows.getAs[Long]("pop_est") == 920938)
      assert(rows.getAs[String]("continent") == "Oceania")
      assert(rows.getAs[String](nameCol) == "Fiji")
      assert(rows.getAs[String]("iso_a3") == "FJI")
      assert(rows.getAs[Double]("gdp_md_est") == 8374.0)
      assert(rows.getAs[Geometry](geometryCol).toString == "MULTIPOLYGON (((180 -16.067132663642447, 180 -16.555216566639196, 179.36414266196414 -16.801354076946883, 178.72505936299711 -17.01204167436804, 178.59683859511713 -16.639150000000004, 179.0966093629971 -16.433984277547403, 179.4135093629971 -16.379054277547404, 180 -16.067132663642447)), ((178.12557 -17.50481, 178.3736 -17.33992, 178.71806 -17.62846, 178.55271 -18.15059, 177.93266000000003 -18.28799, 177.38146 -18.16432, 177.28504 -17.72465, 177.67087 -17.381140000000002, 178.12557 -17.50481)), ((-179.79332010904864 -16.020882256741224, -179.9173693847653 -16.501783135649397, -180 -16.555216566639196, -180 -16.067132663642447, -179.79332010904864 -16.020882256741224)))")
      df.write.format(geoparquetFormat).mode(SaveMode.Overwrite).save(geoparquetoutputlocation + "/gp_sample1.parquet")
      val df2 = sparkSession.read.format(geoparquetFormat).load(geoparquetoutputlocation + "/gp_sample1.parquet")
      val newrows = df2.collect()(0)
      assert(newrows.getAs[Geometry](geometryCol).toString == "MULTIPOLYGON (((180 -16.067132663642447, 180 -16.555216566639196, 179.36414266196414 -16.801354076946883, 178.72505936299711 -17.01204167436804, 178.59683859511713 -16.639150000000004, 179.0966093629971 -16.433984277547403, 179.4135093629971 -16.379054277547404, 180 -16.067132663642447)), ((178.12557 -17.50481, 178.3736 -17.33992, 178.71806 -17.62846, 178.55271 -18.15059, 177.93266000000003 -18.28799, 177.38146 -18.16432, 177.28504 -17.72465, 177.67087 -17.381140000000002, 178.12557 -17.50481)), ((-179.79332010904864 -16.020882256741224, -179.9173693847653 -16.501783135649397, -180 -16.555216566639196, -180 -16.067132663642447, -179.79332010904864 -16.020882256741224)))")
    }
    it("GEOPARQUET Test example2 i.e. naturalearth_citie dataset's Read and Write"){
      val df = sparkSession.read.format(geoparquetFormat).load(geoparquetdatalocation2)
      val rows = df.collect()(0)
      assert(rows.getAs[String](nameCol) == "Vatican City")
      assert(rows.getAs[Geometry](geometryCol).toString == "POINT (12.453386544971766 41.903282179960115)")
      df.write.format(geoparquetFormat).mode(SaveMode.Overwrite).save(geoparquetoutputlocation + "/gp_sample2.parquet")
      val df2 = sparkSession.read.format(geoparquetFormat).load(geoparquetoutputlocation + "/gp_sample2.parquet")
      val newrows = df2.collect()(0)
      assert(newrows.getAs[String](nameCol) == "Vatican City")
      assert(newrows.getAs[Geometry](geometryCol).toString == "POINT (12.453386544971766 41.903282179960115)")
    }
    it("GEOPARQUET Test example3 i.e. nybb dataset's Read and Write"){
      val df = sparkSession.read.format(geoparquetFormat).load(geoparquetdatalocation3)
      val rows = df.collect()(0)
      ...
      assert(rows.getAs[Geometry](geometryCol).toString.startsWith("MULTIPOLYGON (((970217.022"))
      df.write.format(geoparquetFormat).mode(SaveMode.Overwrite).save(geoparquetoutputlocation + "/gp_sample3.parquet")
      val df2 = sparkSession.read.format(geoparquetFormat).load(geoparquetoutputlocation + "/gp_sample3.parquet")
      val newrows = df2.collect()(0)
      assert(newrows.getAs[Geometry](geometryCol).toString.startsWith("MULTIPOLYGON (((970217.022"))
    }
    it("GEOPARQUET Test example-1.0.0-beta.1.parquet") {
      val df = sparkSession.read.format(geoparquetFormat).load(geoparquetdatalocation4)
      val count = df.count()
      val rows = df.collect()
      assert(rows(0).getAs[AnyRef](geometryCol).isInstanceOf[Geometry])
      assert(count == rows.length)
      df.write.format(geoparquetFormat).mode(SaveMode.Overwrite).save(geoparquetoutputlocation + "/gp_sample4.parquet")
      val df2 = sparkSession.read.format(geoparquetFormat).load(geoparquetoutputlocation + "/gp_sample4.parquet")
      val newRows = df2.collect()
      assert(rows.length == newRows.length)
      assert(newRows(0).getAs[AnyRef](geometryCol).isInstanceOf[Geometry])
    ...
    it("GeoParquet with multiple geometry columns") {
      ...
      val schema = StructType(Seq(
        ...
        StructField(g0Col, GeometryUDT, nullable = false),
        StructField(g1Col, GeometryUDT, nullable = false)
      ))
      ...
      df.write.format(geoparquetFormat).save(geoParquetSavePath)
      ...
      parquetFiles.foreach { filePath =>
        ...
        assert(metadata.containsKey(geoMetadataName))
        val geo = parseJson(metadata.get(geoMetadataName))
        implicit val formats : org.json4s.Formats = org.json4s.DefaultFormats
        val g0Types = getGeomTypes(geo, g0Col)
        val g1Types = getGeomTypes(geo, g1Col)
      ...
      val df2 = sparkSession.read.format(geoparquetFormat).load(geoParquetSavePath)
      ...
      assert(rows(0).getAs[AnyRef](g0Col).isInstanceOf[Geometry])
      assert(rows(0).getAs[AnyRef](g1Col).isInstanceOf[Geometry])
    }

    it("GeoParquet save should work with empty dataframes") {
      val schema = StructType(Seq(
        StructField("id", IntegerType, nullable = false),
        StructField(geometryCol, GeometryUDT, nullable = false)
      ))
      ...
      parquetFiles.foreach { filePath =>
        ...
        val geo = parseJson(metadata.get(geoMetadataName))
        implicit val formats: org.json4s.Formats = org.json4s.DefaultFormats
        val g0Types = getGeomTypes(geo, geometryCol)
        val g0BBox = (geo \ "columns" \ geometryCol \ "bbox").extract[Seq[Double]]
    ...
    it("GeoParquet load should raise exception when loading plain parquet files") {
      val e = intercept[SparkException] {
        sparkSession.read.format(geoparquetFormat).load(resourceFolder + "geoparquet/plain.parquet")
      }
    ...
    it("GeoParquet load with spatial predicates") {
      val df = sparkSession.read.format(geoparquetFormat).load(geoparquetdatalocation1)
      val rows = df.where(ST_Intersects(ST_Point(35.174722, -6.552465), col(geometryCol))).collect()
      assert(rows.length == 1)
      assert(rows(0).getAs[String](nameCol) == "Tanzania")
    }
  }
}
